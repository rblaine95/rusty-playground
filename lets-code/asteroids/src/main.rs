...
const VIEWPORT_MAX_X: f32 = VIEWPORT_WIDTH as f32 / 2.;
const VIEWPORT_MIN_X: f32 = -VIEWPORT_MAX_X;
const VIEWPORT_MAX_Y: f32 = VIEWPORT_HEIGHT as f32 / 2.;
const VIEWPORT_MIN_Y: f32 = -VIEWPORT_MAX_Y;

const ASTEROID_VELOCITY: f32 = 2.;

const BULLET_VELOCITY: f32 = 6.;
const BULLET_DISTANCE: f32 = VIEWPORT_HEIGHT as f32 / 1.5;

const STARSHIP_ROTATION_SPEED: f32 = 10. * PI / 360.;
const STARSHIP_ACCELERATION: f32 = 0.2;
const STARSHIP_DECELERATION: f32 = 0.01;
const STARSHIP_MAX_VELOCITY: f32 = 10.;
...
enum AsteroidSize {
    Big,
    Medium, // todo, smaller asteroids are faster
    Small,
}
...
impl Starship {
    fn direction(&self) -> Vec2 {
        let (y, x) = (self.rotation_angle + PI / 2.).sin_cos();
        Vec2::new(x, y)
    }
}
...
fn create_starship_mesh() -> Mesh {
    ...
    mesh.insert_attribute(
        ...
        vec![[0., 0.5, 0.], [-0.25, -0.5, 0.], [0.25, -0.5, 0.]],
    );
    mesh.set_indices(Some(Indices::U32(vec![0, 1, 2])));
    mesh.insert_attribute(Mesh::ATTRIBUTE_NORMAL, vec![[0., 0., 1.]; 3]);
    mesh.insert_attribute(Mesh::ATTRIBUTE_UV_0, vec![[0.5, 0.], [0., 1.], [1., 1.]]);
    mesh
}

fn get_random_point() -> Vec2 {
    Vec2::new(
        rand::random::<f32>() * VIEWPORT_WIDTH as f32,
        rand::random::<f32>() * VIEWPORT_HEIGHT as f32,
    )
}
...
fn setup(
    ...
    commands
        .spawn()
        .insert(Starship { rotation_angle: 0. })
        .insert(Position(Vec2::splat(0.)))
        .insert(Velocity(Vec2::splat(0.)))
    ...
    for _ in 0..6 {
        commands
            ...
            .insert_bundle(MaterialMesh2dBundle {
                ...
                material: materials.add(ColorMaterial::from(Color::rgba(0.8, 0.8, 0.8, 1.))),
                ..default()
            });
    }
}
...
fn update_position(mut query: Query<(&Velocity, &mut Transform, &mut Position)>) {
    for (velocity, transform, mut position) in &mut query {
        let mut new_position = position.0 + velocity.0;
        let half_scale = transform.scale.max_element() / 2.;
            ...
            if let (ButtonState::Pressed, Some(KeyCode::Space)) = (evt.state, evt.key_code) {
                commands
                    ...
                    .insert(Position(Vec2::new(
                        starship_position.0.x,
                        starship_position.0.y,
                    )))
...
fn decelerate_starship(keys: Res<Input<KeyCode>>, mut query: Query<&mut Velocity, With<Starship>>) {
    // only decelerate if we're not accelerating
    if !keys.pressed(KeyCode::Up) {
...
fn detect_starship_asteroid_collision(
    ...
    for (starship_entity, starship_transform, starship_position) in &starship_query {
        for (asteroid_transform, asteroid_position) in &asteroids_query {
            ...
            if distance < starship_size / 4. + asteroid_size / 2. {
...
fn detect_bullet_asteroid_collision(
    ...
    for (bullet_entity, bullet_transform, bullet_position) in &bullets_query {
        for (asteroid_entity, asteroid, asteroid_transform, asteroid_position) in &asteroids_query {
            ...
            if distance < bullet_size / 2. + asteroid_size / 2. {
                ...
                if let Some(asteroid_new_size) = asteroid_new_size {
                    for _ in 0..2 {
                        commands
                            ...
                            .insert(Position(asteroid_position.0))
                            .insert(Velocity(get_random_point().normalize() * ASTEROID_VELOCITY))
                            .insert_bundle(MaterialMesh2dBundle {
                                mesh: meshes.add(Mesh::from(shape::Circle::default())).into(),
                                transform: Transform::default()
                                ...
                                material: materials
                                    .add(ColorMaterial::from(Color::rgba(0.8, 0.8, 0.8, 1.))),
                                ..default()
                            });
                    }
                }
